...
def get_dataset(dataset_name, tokenizer):
    if dataset_name.lower() not in ['snli', 'multi_nli', 'sst2', 'imdb']:
        raise ValueError(f"Invalid dataset name. Expected 'snli', 'multi_nli', 'sst2', or 'imdb', but got {dataset_name}")
...
def model_init_factory(model_name, num_labels, hidden_layers=None):
    def model_init():
...
def save_model(model, training_args, tokenizer):
    model.save_pretrained(training_args.output_dir)
    tokenizer.save_pretrained(training_args.output_dir)


def load_model_and_evaluate(model_dir, dataset, validation_set_name, training_args):
    ...
    for dataset_name in model_names_to_hidden_layers_num:
        for model_name in model_names_to_hidden_layers_num[dataset_name]:
            for hidden_layers in model_names_to_hidden_layers_num[dataset_name][model_name]:
                ...
                dataset, num_labels, validation_set_name = get_dataset(dataset_name, tokenizer)

                # save_directory = f"/content/drive/MyDrive/Colab_Notebooks_RESULTS/ALBERT/ALBERT_{hidden_layers}_{model_name}_{dataset_name}"
                ...
                trainer = Trainer(
                    model_init=model_init_factory(model_name, num_labels, hidden_layers),
                    args=training_args,
                    train_dataset=dataset['train'],
                    eval_dataset=dataset[validation_set_name],
                    compute_metrics=compute_metrics,
                ...
                # Save the model
                save_model(trainer.model, training_args, tokenizer)
                ...
                # model, tokenizer, eval_results = load_model_and_evaluate(save_directory, dataset, validation_set_name, training_args)
